...
namespace CXBILL
{
    public partial class FPayment : UserControl
    {
        ...
        void Editgrid_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            switch (vColName)
            {
                case PAYMENTITEMS.PAYMENT_AMOUNT:
                    CalcTotals();
                    break;
            }
        ...
        private void CalcTotals()
        {
            ...
            decimal vTotal_Payment_amount = 0M;
            for (int r = 0; r <= editgrid.Rows.Count - 1; r++)
            {
                decimal vInvoice_amount = Global.ToDecimal(editgrid[PAYMENTITEMS.PURCHASE_AMOUNT, r].Value);
                vTotal_Invoice_amount += vInvoice_amount;
                decimal vPayment_amount = Global.ToDecimal(editgrid[PAYMENTITEMS.PAYMENT_AMOUNT, r].Value);
                vTotal_Payment_amount += vPayment_amount;
            }

            txt_total_amount.Text = vTotal_Payment_amount.ToString();
        }
        ...
        void FrmLookup_LookupSelected(object sender, EventArgs e)
        {
            ...
            switch (lookupColName)
            {
                case PAYMENTITEMS.PAYMENT_BY:
                    ...
                    if (vEntity == null)
                    {
                        editgrid[PAYMENTITEMS.PAYMENT_BY, lookupRow].Value = "";
                        editgrid[PAYMENTITEMS.PURCHASE_NO, lookupRow].Value = "";
                        editgrid[PAYMENTITEMS.PURCHASE_AMOUNT, lookupRow].Value = "";
                    }
                    else
                    {
                        editgrid[PAYMENTITEMS.PAYMENT_BY, lookupRow].Value = vEntity[PAYMENTITEMS.PAYMENT_BY];

                        if (vEntity[PAYMENTITEMS.PAYMENT_BY].ToString() == Core.Advance)
                        {
                            editgrid[PAYMENTITEMS.PURCHASE_NO, lookupRow].Value = "";
                            editgrid[PAYMENTITEMS.PURCHASE_AMOUNT, lookupRow].Value = "";
                            editgrid[PAYMENTITEMS.PAYMENT_AMOUNT, lookupRow].Value = "";
                            editgrid.CurrentCell = editgrid[PAYMENTITEMS.PURCHASE_AMOUNT, lookupRow];
                        }
                        else
                        {
                            editgrid[PAYMENTITEMS.PURCHASE_NO, lookupRow].Value = vEntity[PAYMENTITEMS.PURCHASE_NO];
                            editgrid[PAYMENTITEMS.PURCHASE_AMOUNT, lookupRow].Value = vEntity[PAYMENTITEMS.PURCHASE_AMOUNT];
                            editgrid[PAYMENTITEMS.PAYMENT_AMOUNT, lookupRow].Value = vEntity[PAYMENTITEMS.PAYMENT_AMOUNT];
                            editgrid.CurrentCell = editgrid[PAYMENTITEMS.PURCHASE_AMOUNT, lookupRow];
                        }
                    }
                    break;
