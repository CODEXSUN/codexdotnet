...
namespace CXBILL
{
    public partial class FPurchase : UserControl
    {
        ...
        void Editgrid_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            switch (vColName)
            {
                case PURCHASEITEMS.QTY:
                    ...
                    CalcTotals();
                    break;
                case PURCHASEITEMS.PRICE:
                    CalcRow(e.RowIndex);
                    CalcTotals();
                    break;
                case PURCHASEITEMS.SGST_PERCENT:
                    CalcRow(e.RowIndex);
                    CalcTotals();
                    break;
                case PURCHASEITEMS.CGST_PERCENT:
                    CalcRow(e.RowIndex);
                    CalcTotals();
                    break;
                case PURCHASEITEMS.IGST_PERCENT:
                    CalcRow(e.RowIndex);
                    CalcTotals();
                    break;
            }
        ...
        private void CalcRow(int pRowIndex)
        {
            ...
            decimal vPrice = ConvertTO.Decimal(editgrid[PURCHASEITEMS.PRICE, pRowIndex].Value);

            decimal vQty = ConvertTO.Decimal(editgrid[PURCHASEITEMS.QTY, pRowIndex].Value);

            decimal vtaxableAmount = vQty * vPrice;

            if (txt_taxtype_id.Text == Core.SGST)
            {
                decimal vSgst_per = ConvertTO.Decimal(editgrid[PURCHASEITEMS.SGST_PERCENT, pRowIndex].Value);
                decimal vCgst_per = ConvertTO.Decimal(editgrid[PURCHASEITEMS.CGST_PERCENT, pRowIndex].Value);
                ...
                editgrid[PURCHASEITEMS.SGST_AMOUNT, pRowIndex].Value = (decimal.Round(vSgst_amount, 2)).ToString();
                editgrid[PURCHASEITEMS.CGST_AMOUNT, pRowIndex].Value = (decimal.Round(vCgst_amount, 2)).ToString();
                editgrid[PURCHASEITEMS.IGST_PERCENT, pRowIndex].Value = "";
                editgrid[PURCHASEITEMS.IGST_AMOUNT, pRowIndex].Value = "";
            }
            else
            {
                decimal vIgst_per = ConvertTO.Decimal(editgrid[PURCHASEITEMS.IGST_PERCENT, pRowIndex].Value);
                decimal vIgst_amount = decimal.Round(((vtaxableAmount * vIgst_per) / 100), 2);

                editgrid[PURCHASEITEMS.SGST_AMOUNT, pRowIndex].Value = "";
                editgrid[PURCHASEITEMS.SGST_PERCENT, pRowIndex].Value = "";
                editgrid[PURCHASEITEMS.CGST_AMOUNT, pRowIndex].Value = "";
                editgrid[PURCHASEITEMS.CGST_PERCENT, pRowIndex].Value = "";
                editgrid[PURCHASEITEMS.IGST_AMOUNT, pRowIndex].Value = (decimal.Round(vIgst_amount, 2)).ToString();
            }

            editgrid[PURCHASEITEMS.TAXABLEAMOUNT, pRowIndex].Value = (decimal.Round(vtaxableAmount, 2)).ToString();
        }
        ...
        private void CalcTotals()
        {
            ...
            {
                vTotQty += ConvertTO.Decimal(editgrid[PURCHASEITEMS.QTY, r].Value);

                vTotTaxable += ConvertTO.Decimal(editgrid[PURCHASEITEMS.TAXABLEAMOUNT, r].Value);

                vTotCgst += ConvertTO.Decimal(editgrid[PURCHASEITEMS.CGST_AMOUNT, r].Value);

                vTotSgst += ConvertTO.Decimal(editgrid[PURCHASEITEMS.SGST_AMOUNT, r].Value);

                vTotIgst += ConvertTO.Decimal(editgrid[PURCHASEITEMS.IGST_AMOUNT, r].Value);
            }
        ...
        {
            editgrid[PURCHASEITEMS.PRODUCT_ID, lookupRow].Value = xproduct.GetName();
            editgrid.Focus();
            editgrid.CurrentCell = editgrid[PURCHASEITEMS.QTY, lookupRow];
