...
namespace CXBILL
{
    public partial class LOffset_labour
    {
        ...
        {
            string pkValue = GetSelectedPkValue();
            if (COffset_labour_exten.LockStatus(pkValue) == false)
            {
                BuildForm();
                if (pkValue != null)
                {
        ...
        {
            txt_no.LookupColNames = new string[] { OFFSET_LABOUR.OFFSET_LABOUR_NO };
            txt_no.LookupUpdate += Txt_no_LookupUpdate;
        }
        void Txt_no_LookupUpdate(object sender, EventArgs e)
        {
            txt_no.LookupList = COffset_labour_exten.LookupNo(new DAL());
        }
        ...
        {
            txt_party.LookupList = COffset_labour_exten.LookupParty(new DAL());
        }
        ...
        private string GetSelectedPkValue()
        {
            ...
            if (vSelRow != null)
            {
                selPkValue = vSelRow.Cells[OFFSET_LABOUR.OFFSET_LABOUR_ID].Value.ToString();
            }
            return selPkValue;
        }
