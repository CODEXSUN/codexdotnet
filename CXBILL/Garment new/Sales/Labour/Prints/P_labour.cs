...
namespace CXBILL
{
    public class P_labour
    {
        ...
        {
            Offset_labour invoice = COffset_labour_exten.PKId(p_id, new DAL());
            ...
            for (int i = 0; i < printcopies.Count; i++)
            {
                ...
                row.INVOICE_ID = p_id;
                row.INVOICE_NO = invoice.Offset_labour_no.ToString();
                row.INVOICE_DATE = ConvertTO.Date2S(invoice.Offset_labour_date);
                ...
                List<Offset_labouritems> klist = COffset_1_invoiceitems_exten.FKId(p_id , new DAL());
