...
namespace CXBILL
{
    public class P_labourItem
    {
        ...
        public static Print_invoiceItemsList GetData(string p_id)
        {
            ...
            //
            List<Offset_labouritems> inv_item_list = new List<Offset_labouritems>();

            //if (Customise.Invoice_poid_dcid == true)
            //{
                inv_item_list = COffset_1_invoiceitems_exten.FKId(p_id, new DAL());
            //}
            ...
            for (int r = 0; r <= inv_item_list.Count - 1; r++)
            {
                ...
                //
                row.INVOICE_ITEM_ID = inv_item_list[r].Offset_labouritems_id;
                row.INVOICE_ID = inv_item_list[r].Offset_labour_id;
                row.SL_NO = (r + 1).ToString();

                if (inv_item_list[r].Offset_labour_po_no == "Without Po")
                {
                    row.PO_ID = "";
                }
                else
                {
                    row.PO_ID = inv_item_list[r].Offset_labour_po_no;
                }


                if (inv_item_list[r].Offset_labour_dc_no == "0")
                {
                    row.DC_ID = "";
                }
                else
                {
                    row.DC_ID = inv_item_list[r].Offset_labour_dc_no;
                }
