...
namespace CXKNITTING
{
    public partial class FInvoice : UserControl
    {
        ...
        void Editgrid_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            ...
            switch (vColName)
            {
                case KNITTING_INVOICEITEMS.QTY:
                    decimal newDecimal;
        ...
        void Editgrid_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            switch (vColName)
            {
                case KNITTING_INVOICEITEMS.QTY:
                    ...
                    CalcTotals();
                    break;
                case KNITTING_INVOICEITEMS.PRICE:
                    CalcRow(e.RowIndex);
                    CalcTotals();
                    break;
                case KNITTING_INVOICEITEMS.SGST_PERCENT:
                    CalcRow(e.RowIndex);
                    CalcTotals();
                    break;
                case KNITTING_INVOICEITEMS.CGST_PERCENT:
                    CalcRow(e.RowIndex);
                    CalcTotals();
                    break;
                case KNITTING_INVOICEITEMS.IGST_PERCENT:
                    CalcRow(e.RowIndex);
                    CalcTotals();
                    break;
            }
        ...
        void Editgrid_CurrentCellChanged(object sender, EventArgs e)
        {
            ...
            if (editgrid.CurrentCell != null)
            {
                ...
                switch (lookupColName)
                {
                    ...
                    case KNITTING_INVOICEITEMS.PRODUCT_ID:
                        ShowLookupForm();
                        break;
                    //case INVOICEITEMS.NOOFROLLS:
                    //    ShowLookupForm();
                    //    break;
                }
            }
        ...
        private void CalcRow(int pRowIndex)
        {
            ...
            decimal vPrice = ConvertTO.Decimal(editgrid[KNITTING_INVOICEITEMS.PRICE, pRowIndex].Value);

            decimal vQty = ConvertTO.Decimal(editgrid[KNITTING_INVOICEITEMS.QTY, pRowIndex].Value);

            decimal vtaxableAmount = vQty * vPrice;

            if (txt_taxtype_id.Text == Core.SGST)
            {
                decimal vSgst_per = ConvertTO.Decimal(editgrid[KNITTING_INVOICEITEMS.SGST_PERCENT, pRowIndex].Value);
                decimal vCgst_per = ConvertTO.Decimal(editgrid[KNITTING_INVOICEITEMS.CGST_PERCENT, pRowIndex].Value);
                ...
                editgrid[KNITTING_INVOICEITEMS.SGST_AMOUNT, pRowIndex].Value = (decimal.Round(vSgst_amount, 2)).ToString();
                editgrid[KNITTING_INVOICEITEMS.CGST_AMOUNT, pRowIndex].Value = (decimal.Round(vCgst_amount, 2)).ToString();
                editgrid[KNITTING_INVOICEITEMS.IGST_PERCENT, pRowIndex].Value = "";
                editgrid[KNITTING_INVOICEITEMS.IGST_AMOUNT, pRowIndex].Value = "";
            }
            else
            {
                decimal vIgst_per = ConvertTO.Decimal(editgrid[KNITTING_INVOICEITEMS.IGST_PERCENT, pRowIndex].Value);
                decimal vIgst_amount = decimal.Round(((vtaxableAmount * vIgst_per) / 100), 2);

                editgrid[KNITTING_INVOICEITEMS.SGST_AMOUNT, pRowIndex].Value = "";
                editgrid[KNITTING_INVOICEITEMS.SGST_PERCENT, pRowIndex].Value = "";
                editgrid[KNITTING_INVOICEITEMS.CGST_AMOUNT, pRowIndex].Value = "";
                editgrid[KNITTING_INVOICEITEMS.CGST_PERCENT, pRowIndex].Value = "";
                editgrid[KNITTING_INVOICEITEMS.IGST_AMOUNT, pRowIndex].Value = (decimal.Round(vIgst_amount, 2)).ToString();
            }

            editgrid[KNITTING_INVOICEITEMS.TAXABLEAMOUNT, pRowIndex].Value = (decimal.Round(vtaxableAmount, 2)).ToString();
        }
        ...
        private void CalcTotals()
        {
            ...
            {
                vTotQty += ConvertTO.Decimal(editgrid[KNITTING_INVOICEITEMS.QTY, r].Value);

                vrolls += ConvertTO.Decimal(editgrid[KNITTING_INVOICEITEMS.NOOFROLLS, r].Value);

                vTotTaxable += ConvertTO.Decimal(editgrid[KNITTING_INVOICEITEMS.TAXABLEAMOUNT, r].Value);

                vTotCgst += ConvertTO.Decimal(editgrid[KNITTING_INVOICEITEMS.CGST_AMOUNT, r].Value);

                vTotSgst += ConvertTO.Decimal(editgrid[KNITTING_INVOICEITEMS.SGST_AMOUNT, r].Value);

                vTotIgst += ConvertTO.Decimal(editgrid[KNITTING_INVOICEITEMS.IGST_AMOUNT, r].Value);
            }
        ...
        public void ShowLookupForm()
        {
            ...
            switch (lookupColName)
            {
                ...
                case KNITTING_INVOICEITEMS.PRODUCT_ID:
                    {
                        ...
                        frmLookup.SelectedPkValue = editgrid[KNITTING_INVOICEITEMS.PRODUCT_ID, lookupRow].Value;
                ...
                //    frmLookup.SelectedPkValue = editgrid[INVOICEITEMS.NOOFROLLS, lookupRow].Value;
                //    frmLookup.AllowNewEntry = true;
                //    frmLookup.AllowEmptySelection = true;
        ...
        void FrmLookup_LookupSelected(object sender, EventArgs e)
        {
            ...
            switch (lookupColName)
            {
                ...
                case KNITTING_INVOICEITEMS.PRODUCT_ID:
                    if (vEntity == null)
                    {
                        editgrid[KNITTING_INVOICEITEMS.PRODUCT_ID, lookupRow].Value = "";
                    }
                    else
                    {

                        editgrid[KNITTING_INVOICEITEMS.PRODUCT_ID, lookupRow].Value = Checks.RemoveEscape(vEntity[PRODUCT.PRODUCT_NAME].ToString());
                        editgrid.CurrentCell = editgrid[KNITTING_INVOICEITEMS.PRODUCT_ID, lookupRow];

                        if (txt_taxtype_id.Text == "SGST")
                        {
                            editgrid[KNITTING_INVOICEITEMS.SGST_PERCENT, lookupRow].Value = Ccoreplus.GetSgst_Name(vEntity[PRODUCT.PRODUCT_ID] + "");
                            editgrid[KNITTING_INVOICEITEMS.CGST_PERCENT, lookupRow].Value = Ccoreplus.GetCgst_Name(vEntity[PRODUCT.PRODUCT_ID] + "");
                        }
                        else if (txt_taxtype_id.Text == "IGST")
                        {
                            editgrid[KNITTING_INVOICEITEMS.IGST_PERCENT, lookupRow].Value = Ccoreplus.GetIgst_Name(vEntity[PRODUCT.PRODUCT_ID] + "");
                        }
                    }
                    break;
                ...
                //    if (vEntity == null)
                //    {
                //        editgrid[INVOICEITEMS.NOOFROLLS, lookupRow].Value = "";
                //    }
                //    else
                //    {
                //        editgrid[INVOICEITEMS.NOOFROLLS, lookupRow].Value = vEntity[SIZES.SIZES_NAME];
                //    }
                //    break;
            }
        }
        ...
        {
            editgrid[KNITTING_INVOICEITEMS.PRODUCT_ID, lookupRow].Value = xproduct.GetName();
            editgrid.Focus();
            editgrid.CurrentCell = editgrid[KNITTING_INVOICEITEMS.NOOFROLLS, lookupRow];
        ...
        {
            editgrid[KNITTING_INVOICEITEMS.NOOFROLLS, lookupRow].Value = xsizes.GetName();
            editgrid.Focus();
            editgrid.CurrentCell = editgrid[KNITTING_INVOICEITEMS.QTY, lookupRow];
