...
namespace CXKNITTING
{
    public partial class LInvoice 
    {
        ...
        {
            string pkValue = GetSelectedPkValue();
            if (CInvoice_exten.LockStatus(pkValue) == false)
            {
                BuildForm();
                if (pkValue != null)
                {
        ...
        {
            txt_no.LookupColNames = new string[] { KNITTING_INVOICE.KNITTING_INVOICE_NO };
            txt_no.LookupUpdate += Txt_no_LookupUpdate;
        }
        void Txt_no_LookupUpdate(object sender, EventArgs e)
        {
            txt_no.LookupList = CInvoice_exten.LookupNo(new DAL());
        }
        ...
        {
            txt_party.LookupList = CInvoice_exten.LookupParty(new DAL());
        }
        ...
        private string GetSelectedPkValue()
        {
            ...
            if (vSelRow != null)
            {
                selPkValue = vSelRow.Cells[KNITTING_INVOICE.KNITTING_INVOICE_ID].Value.ToString();
            }
            return selPkValue;
        }
