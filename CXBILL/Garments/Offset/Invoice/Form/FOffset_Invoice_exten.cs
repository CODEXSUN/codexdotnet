    ...
    public partial class FOffset_Invoice : UserControl
    {
        ...
        void Editgrid_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            switch (vColName)
            {
                case OFFSET_INVOICEITEMS.QTY:
                    ...
                    CalcTotals();
                    break;
                case OFFSET_INVOICEITEMS.PRICE:
                    CalcRow(e.RowIndex);
                    CalcTotals();
                    break;
                case OFFSET_INVOICEITEMS.SGST_PERCENT:
                    CalcRow(e.RowIndex);
                    CalcTotals();
                    break;
                case OFFSET_INVOICEITEMS.CGST_PERCENT:
                    CalcRow(e.RowIndex);
                    CalcTotals();
                    break;
                case OFFSET_INVOICEITEMS.IGST_PERCENT:
                    CalcRow(e.RowIndex);
                    CalcTotals();
                    break;
            }
        ...
        void Editgrid_CurrentCellChanged(object sender, EventArgs e)
        {
            ...
            if (editgrid.CurrentCell != null)
            {
                ...
                {
                    case OFFSET_INVOICEITEMS.OFFSET_PO_ID:
                        ShowLookupForm();
                        break;
                    case OFFSET_INVOICEITEMS.OFFSET_DC_ID:
                        ShowLookupForm();
                        break;
                    case OFFSET_INVOICEITEMS.PRODUCT_ID:
                        ShowLookupForm();
                        break;
                }
            }
        ...
        private void CalcRow(int pRowIndex)
        {
            ...
            decimal vPrice = ConvertTO.Decimal(editgrid[OFFSET_INVOICEITEMS.PRICE, pRowIndex].Value);

            decimal vQty = ConvertTO.Decimal(editgrid[OFFSET_INVOICEITEMS.QTY, pRowIndex].Value);

            decimal vtaxableAmount = vQty * vPrice;

            if (txt_taxtype_id.Text == Core.SGST)
            {
                decimal vSgst_per = ConvertTO.Decimal(editgrid[OFFSET_INVOICEITEMS.SGST_PERCENT, pRowIndex].Value);
                decimal vCgst_per = ConvertTO.Decimal(editgrid[OFFSET_INVOICEITEMS.CGST_PERCENT, pRowIndex].Value);
                ...
                editgrid[OFFSET_INVOICEITEMS.SGST_AMOUNT, pRowIndex].Value = (decimal.Round(vSgst_amount, 2)).ToString();
                editgrid[OFFSET_INVOICEITEMS.CGST_AMOUNT, pRowIndex].Value = (decimal.Round(vCgst_amount, 2)).ToString();
                editgrid[OFFSET_INVOICEITEMS.IGST_PERCENT, pRowIndex].Value = "";
                editgrid[OFFSET_INVOICEITEMS.IGST_AMOUNT, pRowIndex].Value = "";
            }
            else
            {
                decimal vIgst_per = ConvertTO.Decimal(editgrid[OFFSET_INVOICEITEMS.IGST_PERCENT, pRowIndex].Value);
                decimal vIgst_amount = decimal.Round(((vtaxableAmount * vIgst_per) / 100), 2);

                editgrid[OFFSET_INVOICEITEMS.SGST_AMOUNT, pRowIndex].Value = "";
                editgrid[OFFSET_INVOICEITEMS.SGST_PERCENT, pRowIndex].Value = "";
                editgrid[OFFSET_INVOICEITEMS.CGST_AMOUNT, pRowIndex].Value = "";
                editgrid[OFFSET_INVOICEITEMS.CGST_PERCENT, pRowIndex].Value = "";
                editgrid[OFFSET_INVOICEITEMS.IGST_AMOUNT, pRowIndex].Value = (decimal.Round(vIgst_amount, 2)).ToString();
            }

            editgrid[OFFSET_INVOICEITEMS.TAXABLEAMOUNT, pRowIndex].Value = (decimal.Round(vtaxableAmount, 2)).ToString();
        }
        ...
        private void CalcTotals()
        {
            ...
            {
                vTotQty += ConvertTO.Decimal(editgrid[OFFSET_INVOICEITEMS.QTY, r].Value);

                vTotTaxable += ConvertTO.Decimal(editgrid[OFFSET_INVOICEITEMS.TAXABLEAMOUNT, r].Value);

                vTotCgst += ConvertTO.Decimal(editgrid[OFFSET_INVOICEITEMS.CGST_AMOUNT, r].Value);

                vTotSgst += ConvertTO.Decimal(editgrid[OFFSET_INVOICEITEMS.SGST_AMOUNT, r].Value);

                vTotIgst += ConvertTO.Decimal(editgrid[OFFSET_INVOICEITEMS.IGST_AMOUNT, r].Value);
            }
                ...
                case OFFSET_INVOICEITEMS.OFFSET_PO_ID:
                    {
                        ...
                        frmLookup.LookupColNames = new string[] { OFFSET_PO.OFFSET_PO_NO, OFFSET_DC.OFFSET_DC_NO, OFFSET_DCITEMS.PRODUCT_ID, OFFSET_DCITEMS.QTY, OFFSET_DCITEMS.REFERED_QTY, OFFSET_POITEMS.PRICE };
                        frmLookup.SelectedPkValue = editgrid[OFFSET_INVOICEITEMS.OFFSET_PO_ID, lookupRow].Value;
                ...
                case OFFSET_INVOICEITEMS.OFFSET_DC_ID:
                    {
                        ...
                        frmLookup.LookupColNames = new string[] { OFFSET_DC.OFFSET_DC_NO, OFFSET_DCITEMS.PRODUCT_ID, OFFSET_DCITEMS.QTY, OFFSET_DCITEMS.REFERED_QTY, OFFSET_POITEMS.PRICE };
                        frmLookup.SelectedPkValue = editgrid[OFFSET_INVOICEITEMS.OFFSET_DC_ID, lookupRow].Value;
                        frmLookup.AllowNewEntry = true;
                        frmLookup.AllowEmptySelection = true;
                        ...
                        if (txt_party_id.Text != "")
                        {
                            ...
                            {
                                if (editgrid[OFFSET_DCITEMS.OFFSET_PO_ID, lookupRow].Value.ToString() != "")
                                {
                                    if (editgrid[OFFSET_DCITEMS.OFFSET_PO_ID, lookupRow].Value.ToString() != Core.Without_Po)
                                    { }
                                    else
                                    {
                                        frmLookup.LookupList = COffset_dc_Lookup.LookupDC_Party_forInvoice(CParty_exten.GetId_Name(txt_party_id.Text));
                                        frmLookup.LoadLookupList();
                                    }
                                }
                                else
                ...
                case OFFSET_INVOICEITEMS.PRODUCT_ID:
                    {
                        ...
                        frmLookup.SelectedPkValue = editgrid[OFFSET_INVOICEITEMS.PRODUCT_ID, lookupRow].Value;
                        frmLookup.AllowNewEntry = true;
                        frmLookup.AllowEmptySelection = true;
                        ...
                        if (txt_party_id.Text != "")
                        {
                            ...
                            if (editgrid[OFFSET_DCITEMS.OFFSET_PO_ID, lookupRow].Value != null)
                            {
                                if (editgrid[OFFSET_DCITEMS.OFFSET_DC_ID, lookupRow].Value != null)
                                {

                                    if (editgrid[OFFSET_DCITEMS.OFFSET_PO_ID, lookupRow].Value.ToString() == Core.Without_Po)
                                    {
                                        if (editgrid[OFFSET_DCITEMS.OFFSET_DC_ID, lookupRow].Value.ToString() == Core.Without_Dc)
                                        {
        ...
        void FrmLookup_LookupSelected(object sender, EventArgs e)
        {
            ...
            switch (lookupColName)
            {
                case OFFSET_INVOICEITEMS.OFFSET_PO_ID:
                    if (vEntity == null)
                    {
                        editgrid[OFFSET_INVOICEITEMS.OFFSET_PO_ID, lookupRow].Value = "";
                    }
                    else
                    {
                        editgrid[OFFSET_INVOICEITEMS.OFFSET_PO_ID, lookupRow].Value = vEntity[OFFSET_PO.OFFSET_PO_NO];
                        if (vEntity[OFFSET_PO.OFFSET_PO_NO] + "" != Core.Without_Po)
                        {
                            editgrid[OFFSET_INVOICEITEMS.OFFSET_DC_ID, lookupRow].Value = vEntity[OFFSET_DC.OFFSET_DC_NO];
                            editgrid[OFFSET_INVOICEITEMS.OFFSET_DCITEMS_ID, lookupRow].Value = vEntity[OFFSET_DCITEMS.OFFSET_DCITEMS_ID];
                            editgrid[OFFSET_INVOICEITEMS.PRODUCT_ID, lookupRow].Value = vEntity[OFFSET_DCITEMS.PRODUCT_ID];
                            editgrid[OFFSET_INVOICEITEMS.QTY, lookupRow].Value = vEntity[OFFSET_DCITEMS.QTY];
                            editgrid[OFFSET_INVOICEITEMS.PRICE, lookupRow].Value = vEntity[OFFSET_POITEMS.PRICE];

                            //if (txt_taxtype_id.Text == "SGST")
                            //{
                            //    editgrid[OFFSET_INVOICEITEMS.SGST_PERCENT, lookupRow].Value = Ccoreplus.GetSgst_Name(vEntity[PRODUCT.PRODUCT_ID] + "");
                            //    editgrid[OFFSET_INVOICEITEMS.CGST_PERCENT, lookupRow].Value = Ccoreplus.GetCgst_Name(vEntity[PRODUCT.PRODUCT_ID] + "");
                            //}
                            //else if (txt_taxtype_id.Text == "IGST")
                            //{
                            //    editgrid[OFFSET_INVOICEITEMS.IGST_PERCENT, lookupRow].Value = Ccoreplus.GetIgst_Name(vEntity[PRODUCT.PRODUCT_ID] + "");
                            //}
                            editgrid.CurrentCell = editgrid[OFFSET_INVOICEITEMS.PRICE, lookupRow];
                        }
                        else
                        {
                            editgrid.CurrentCell = editgrid[OFFSET_INVOICEITEMS.OFFSET_PO_ID, lookupRow];
                        }
                    }
                    break;
                ...
                case OFFSET_INVOICEITEMS.OFFSET_DC_ID:
                    if (vEntity == null)
                    {
                        editgrid[OFFSET_INVOICEITEMS.OFFSET_DC_ID, lookupRow].Value = "";
                    }
                    else
                    {
                        editgrid[OFFSET_INVOICEITEMS.OFFSET_DC_ID, lookupRow].Value = vEntity[OFFSET_DC.OFFSET_DC_NO];
                        if (vEntity[OFFSET_DCITEMS.OFFSET_DC_NO] + "" != Core.Without_Dc)
                        {
                            editgrid[OFFSET_INVOICEITEMS.OFFSET_PO_ID, lookupRow].Value = Core.Without_Po;
                            //editgrid[INVOICEITEMS.DCITEM_ID, lookupRow].Value = vEntity[Dc_item.DC_ITEM_ID];
                            editgrid[OFFSET_INVOICEITEMS.PRODUCT_ID, lookupRow].Value = vEntity[OFFSET_DCITEMS.PRODUCT_ID];
                            editgrid[OFFSET_INVOICEITEMS.QTY, lookupRow].Value = vEntity[OFFSET_DCITEMS.REFERED_QTY];
                            editgrid[OFFSET_INVOICEITEMS.PRICE, lookupRow].Value = vEntity[OFFSET_POITEMS.PRICE];

                            editgrid.CurrentCell = editgrid[OFFSET_INVOICEITEMS.PRODUCT_ID, lookupRow];
                        }
                        else
                        {
                            editgrid.CurrentCell = editgrid[OFFSET_INVOICEITEMS.OFFSET_DC_ID, lookupRow];
                        }
                    }
                    break;
                ...
                case OFFSET_INVOICEITEMS.PRODUCT_ID:
                    if (vEntity == null)
                    {
                        editgrid[OFFSET_INVOICEITEMS.PRODUCT_ID, lookupRow].Value = "";
                    }
                    else
                    {

                        editgrid[OFFSET_INVOICEITEMS.PRODUCT_ID, lookupRow].Value = Checks.RemoveEscape(vEntity[PRODUCT.PRODUCT_NAME].ToString());
                        editgrid.CurrentCell = editgrid[OFFSET_INVOICEITEMS.PRODUCT_ID, lookupRow];
        ...
        {
            editgrid[OFFSET_INVOICEITEMS.PRODUCT_ID, lookupRow].Value = xproduct.GetName();
            editgrid.Focus();
            editgrid.CurrentCell = editgrid[OFFSET_INVOICEITEMS.PRICE, lookupRow];
