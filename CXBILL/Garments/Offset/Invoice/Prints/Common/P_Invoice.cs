...
namespace CXBILL
{
    public class P_Invoice
    {
        ...
        {
            Offset_invoice invoice = COffset_invoice_exten.PKId(p_id, new DAL());
            ...
            for (int i = 0; i < printcopies.Count; i++)
            {
                ...
                row.INVOICE_ID = p_id;
                row.INVOICE_NO = invoice.Offset_invoice_no.ToString();
                row.INVOICE_DATE = ConvertTO.Date2S(invoice.Offset_invoice_date);
                ...
                List<Offset_invoiceitems> klist = COffset_invoiceitems_exten.FKId(p_id , new DAL());
