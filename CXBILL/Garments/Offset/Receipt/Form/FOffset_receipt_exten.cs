...
namespace CXBILL
{
    public partial class FOffset_receipt : UserControl
    {
        ...
        void Editgrid_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            switch (vColName)
            {
                case OFFSET_RECEIPTITEMS.OFFSET_RECEIPT_AMOUNT:
                    CalcTotals();
                    break;
            }
        ...
        void Editgrid_CurrentCellChanged(object sender, EventArgs e)
        {
            ...
            if (editgrid.CurrentCell != null)
            {
                ...
                {
                    case OFFSET_RECEIPTITEMS.OFFSET_RECEIPT_BY:
                        ShowLookupForm();
                        break;
                }
            }
        ...
        private void CalcTotals()
        {
            ...
            {
                decimal vInvoice_amount = Global.ToDecimal(editgrid[OFFSET_RECEIPTITEMS.OFFSET_INVOICED_AMOUNT, r].Value);
                vTotal_Invoice_amount += vInvoice_amount;
                decimal vReceipt_amount = Global.ToDecimal(editgrid[OFFSET_RECEIPTITEMS.OFFSET_RECEIPT_AMOUNT, r].Value);
        ...
        public void ShowLookupForm()
        {
            ...
            switch (lookupColName)
            {
                ...
                case OFFSET_RECEIPTITEMS.OFFSET_RECEIPT_BY:
                    {
                        frmLookup.LookupColNames = new string[] { OFFSET_RECEIPTITEMS.OFFSET_RECEIPT_BY, OFFSET_RECEIPTITEMS.OFFSET_INVOICED_NO, OFFSET_RECEIPTITEMS.OFFSET_INVOICED_AMOUNT, OFFSET_RECEIPTITEMS.OFFSET_RECEIPT_AMOUNT };
                        frmLookup.SelectedPkValue = editgrid[OFFSET_RECEIPTITEMS.OFFSET_RECEIPT_BY, lookupRow].Value;
        ...
        void FrmLookup_LookupSelected(object sender, EventArgs e)
        {
            ...
            switch (lookupColName)
            {
                case OFFSET_RECEIPTITEMS.OFFSET_RECEIPT_BY:
                    ...
                    if (vEntity == null)
                    {
                        editgrid[OFFSET_RECEIPTITEMS.OFFSET_RECEIPT_BY, lookupRow].Value = "";
                        editgrid[OFFSET_RECEIPTITEMS.OFFSET_INVOICED_NO, lookupRow].Value = "";
                        editgrid[OFFSET_RECEIPTITEMS.OFFSET_INVOICED_AMOUNT, lookupRow].Value = "";
                    }
                    else
                    {
                        editgrid[OFFSET_RECEIPTITEMS.OFFSET_RECEIPT_BY, lookupRow].Value = vEntity[OFFSET_RECEIPTITEMS.OFFSET_RECEIPT_BY];

                        if (vEntity[OFFSET_RECEIPTITEMS.OFFSET_RECEIPT_BY].ToString() == Core.Advance)
                        {
                            editgrid[OFFSET_RECEIPTITEMS.OFFSET_INVOICED_NO, lookupRow].Value = "";
                            editgrid[OFFSET_RECEIPTITEMS.OFFSET_INVOICED_AMOUNT, lookupRow].Value = "";
                            editgrid[OFFSET_RECEIPTITEMS.OFFSET_RECEIPT_AMOUNT, lookupRow].Value = "";
                            editgrid.CurrentCell = editgrid[OFFSET_RECEIPTITEMS.OFFSET_INVOICED_AMOUNT, lookupRow];
                        }
                        else
                        {
                            editgrid[OFFSET_RECEIPTITEMS.OFFSET_INVOICED_NO, lookupRow].Value = vEntity[OFFSET_RECEIPTITEMS.OFFSET_INVOICED_NO];
                            editgrid[OFFSET_RECEIPTITEMS.OFFSET_INVOICED_AMOUNT, lookupRow].Value = vEntity[OFFSET_RECEIPTITEMS.OFFSET_INVOICED_AMOUNT];
                            editgrid[OFFSET_RECEIPTITEMS.OFFSET_RECEIPT_AMOUNT, lookupRow].Value = vEntity[OFFSET_RECEIPTITEMS.OFFSET_RECEIPT_AMOUNT];
                            editgrid.CurrentCell = editgrid[OFFSET_RECEIPTITEMS.OFFSET_INVOICED_AMOUNT, lookupRow];
                        }
                    }
                    break;
