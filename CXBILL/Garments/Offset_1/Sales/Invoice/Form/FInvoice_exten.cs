...
namespace CXBILL
{
    public partial class FOffset_1_Invoice : UserControl
    {
        ...
        void Editgrid_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            ...
            switch (vColName)
            {
                case OFFSET_1_INVOICEITEMS.QTY:
                    decimal newDecimal;
        ...
        void Editgrid_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            switch (vColName)
            {
                case OFFSET_1_INVOICEITEMS.QTY:
                    ...
                    CalcTotals();
                    break;
                case OFFSET_1_INVOICEITEMS.PRICE:
                    CalcRow(e.RowIndex);
                    CalcTotals();
                    break;
                case OFFSET_1_INVOICEITEMS.SGST_PERCENT:
                    CalcRow(e.RowIndex);
                    CalcTotals();
                    break;
                case OFFSET_1_INVOICEITEMS.CGST_PERCENT:
                    CalcRow(e.RowIndex);
                    CalcTotals();
                    break;
                case OFFSET_1_INVOICEITEMS.IGST_PERCENT:
                    CalcRow(e.RowIndex);
                    CalcTotals();
                    break;
            }
        ...
        void Editgrid_CurrentCellChanged(object sender, EventArgs e)
        {
            ...
            if (editgrid.CurrentCell != null)
            {
                ...
                {
                    case OFFSET_1_INVOICEITEMS.PRODUCT_ID:
                        ShowLookupForm();
                        break;
                }
            }
        ...
        private void CalcRow(int pRowIndex)
        {
            ...
            decimal vPrice = ConvertTO.Decimal(editgrid[OFFSET_1_INVOICEITEMS.PRICE, pRowIndex].Value);

            decimal vQty = ConvertTO.Decimal(editgrid[OFFSET_1_INVOICEITEMS.QTY, pRowIndex].Value);

            decimal vtaxableAmount = vQty * vPrice;

            if (txt_taxtype_id.Text == Core.SGST)
            {
                decimal vSgst_per = ConvertTO.Decimal(editgrid[OFFSET_1_INVOICEITEMS.SGST_PERCENT, pRowIndex].Value);
                decimal vCgst_per = ConvertTO.Decimal(editgrid[OFFSET_1_INVOICEITEMS.CGST_PERCENT, pRowIndex].Value);
                ...
                editgrid[OFFSET_1_INVOICEITEMS.SGST_AMOUNT, pRowIndex].Value = (decimal.Round(vSgst_amount, 2)).ToString();
                editgrid[OFFSET_1_INVOICEITEMS.CGST_AMOUNT, pRowIndex].Value = (decimal.Round(vCgst_amount, 2)).ToString();
                editgrid[OFFSET_1_INVOICEITEMS.IGST_PERCENT, pRowIndex].Value = "";
                editgrid[OFFSET_1_INVOICEITEMS.IGST_AMOUNT, pRowIndex].Value = "";
            }
            else
            {
                decimal vIgst_per = ConvertTO.Decimal(editgrid[OFFSET_1_INVOICEITEMS.IGST_PERCENT, pRowIndex].Value);
                decimal vIgst_amount = decimal.Round(((vtaxableAmount * vIgst_per) / 100), 2);

                editgrid[OFFSET_1_INVOICEITEMS.SGST_AMOUNT, pRowIndex].Value = "";
                editgrid[OFFSET_1_INVOICEITEMS.SGST_PERCENT, pRowIndex].Value = "";
                editgrid[OFFSET_1_INVOICEITEMS.CGST_AMOUNT, pRowIndex].Value = "";
                editgrid[OFFSET_1_INVOICEITEMS.CGST_PERCENT, pRowIndex].Value = "";
                editgrid[OFFSET_1_INVOICEITEMS.IGST_AMOUNT, pRowIndex].Value = (decimal.Round(vIgst_amount, 2)).ToString();
            }

            editgrid[OFFSET_1_INVOICEITEMS.TAXABLEAMOUNT, pRowIndex].Value = (decimal.Round(vtaxableAmount, 2)).ToString();
        }
        ...
        private void CalcTotals()
        {
            ...
            {
                vTotQty += ConvertTO.Decimal(editgrid[OFFSET_1_INVOICEITEMS.QTY, r].Value);

                vTotTaxable += ConvertTO.Decimal(editgrid[OFFSET_1_INVOICEITEMS.TAXABLEAMOUNT, r].Value);

                vTotCgst += ConvertTO.Decimal(editgrid[OFFSET_1_INVOICEITEMS.CGST_AMOUNT, r].Value);

                vTotSgst += ConvertTO.Decimal(editgrid[OFFSET_1_INVOICEITEMS.SGST_AMOUNT, r].Value);

                vTotIgst += ConvertTO.Decimal(editgrid[OFFSET_1_INVOICEITEMS.IGST_AMOUNT, r].Value);
            }
                ...
                case OFFSET_1_INVOICEITEMS.PRODUCT_ID:
                    {
                        ...
                        frmLookup.SelectedPkValue = editgrid[OFFSET_1_INVOICEITEMS.PRODUCT_ID, lookupRow].Value;
        ...
        void FrmLookup_LookupSelected(object sender, EventArgs e)
        {
            ...
            switch (lookupColName)
            {
                case OFFSET_1_INVOICEITEMS.PRODUCT_ID:
                    if (vEntity == null)
                    {
                        editgrid[OFFSET_1_INVOICEITEMS.PRODUCT_ID, lookupRow].Value = "";
                    }
                    else
                    {

                        editgrid[OFFSET_1_INVOICEITEMS.PRODUCT_ID, lookupRow].Value = Checks.RemoveEscape(vEntity[PRODUCT.PRODUCT_NAME].ToString());
                        editgrid.CurrentCell = editgrid[OFFSET_1_INVOICEITEMS.PRODUCT_ID, lookupRow];

                        if (txt_taxtype_id.Text == "SGST")
                        {
                            editgrid[OFFSET_1_INVOICEITEMS.SGST_PERCENT, lookupRow].Value = Ccoreplus.GetSgst_Name(vEntity[PRODUCT.PRODUCT_ID] + "");
                            editgrid[OFFSET_1_INVOICEITEMS.CGST_PERCENT, lookupRow].Value = Ccoreplus.GetCgst_Name(vEntity[PRODUCT.PRODUCT_ID] + "");
                        }
                        else if (txt_taxtype_id.Text == "IGST")
                        {
                            editgrid[OFFSET_1_INVOICEITEMS.IGST_PERCENT, lookupRow].Value = Ccoreplus.GetIgst_Name(vEntity[PRODUCT.PRODUCT_ID] + "");
                        }
                    }
                    break;
        ...
        {
            editgrid[OFFSET_1_INVOICEITEMS.PRODUCT_ID, lookupRow].Value = xproduct.GetName();
            editgrid.Focus();
            editgrid.CurrentCell = editgrid[OFFSET_1_INVOICEITEMS.PRICE, lookupRow];
