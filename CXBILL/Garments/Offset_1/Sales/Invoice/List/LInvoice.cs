...
namespace CXBILL
{
    public partial class LOffset_1_invoice
    {
        ...
        void Listgrid_RowAction(object sender, EventArgs e)
        {
            ...
            if (finvoice == null)
            {
                finvoice = new FOffset_1_Invoice();
            }
            BuildForm();
        ...
        {
            string pkValue = GetSelectedPkValue();
            if (COffset_1_invoice_exten.LockStatus(pkValue) == false)
            {
                BuildForm();
                if (pkValue != null)
                {
        ...
        {
            txt_no.LookupColNames = new string[] { OFFSET_1_INVOICE.OFFSET_1_INVOICE_NO };
            txt_no.LookupUpdate += Txt_no_LookupUpdate;
        }
        void Txt_no_LookupUpdate(object sender, EventArgs e)
        {
            txt_no.LookupList = COffset_1_invoice_exten.LookupNo(new DAL());
        }
        ...
        {
            txt_party.LookupList = COffset_1_invoice_exten.LookupParty(new DAL());
        }
        ...
        private string GetSelectedPkValue()
        {
            ...
            if (vSelRow != null)
            {
                selPkValue = vSelRow.Cells[OFFSET_1_INVOICE.OFFSET_1_INVOICE_ID].Value.ToString();
            }
            return selPkValue;
        }
