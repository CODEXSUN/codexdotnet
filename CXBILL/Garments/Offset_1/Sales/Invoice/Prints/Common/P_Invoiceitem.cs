...
namespace CXBILL
{
    public class P_InvoiceItem
    {
        ...
        public static Print_invoiceItemsList GetData(string p_id)
        {
            ...
            //
            List<Offset_1_invoiceitems> inv_item_list = new List<Offset_1_invoiceitems>();
            ...
            for (int r = 0; r <= inv_item_list.Count - 1; r++)
            {
                ...
                //
                row.INVOICE_ITEM_ID = inv_item_list[r].Offset_1_invoiceitems_id;
                row.INVOICE_ID = inv_item_list[r].Offset_1_invoice_id;
                row.SL_NO = (r + 1).ToString();

                if (inv_item_list[r].Offset_1_po_no == "Without Po")
                {
                    row.PO_ID = "";
                }
                else
                {
                    row.PO_ID = inv_item_list[r].Offset_1_po_no;
                }


                if (inv_item_list[r].Offset_1_dc_no == "0")
                {
                    row.DC_ID = "";
                }
                else
                {
                    row.DC_ID = inv_item_list[r].Offset_1_dc_no;
                }
