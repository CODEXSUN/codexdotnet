...
namespace CXBILL
{
    public class P_InvoiceItem
    {
        ...
        public static Print_invoiceItemsList GetData(string p_id)
        {
            ...
            //
            List<Garment_invoiceitems> inv_item_list = new List<Garment_invoiceitems>();
            inv_item_list = CGarment_invoiceitems_exten.FKId(p_id);

            for (int r = 0; r <= inv_item_list.Count - 1; r++)
            {
                Print_invoiceitemsRow row = new Print_invoiceitemsRow();
                //
                row.INVOICE_ITEM_ID = inv_item_list[r].Garment_invoiceitems_id;
                row.INVOICE_ID = inv_item_list[r].Garment_invoice_id;
                ...
                {
                    row.QTY = ConvertTO.Decimal3d(inv_item_list[r].Qty.ToString());
                }
