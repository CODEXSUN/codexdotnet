...
namespace CXBILL
{
    public partial class FTex_Invoice 
    {
        ...
        void Editgrid_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            switch (vColName)
            {
                case TEX_INVOICEITEMS.QTY:
                    ...
                    CalcTotals();
                    break;
                case TEX_INVOICEITEMS.PRICE:
                    CalcRow(e.RowIndex);
                    CalcTotals();
                    break;
                case TEX_INVOICEITEMS.SGST_PERCENT:
                    CalcRow(e.RowIndex);
                    CalcTotals();
                    break;
                case TEX_INVOICEITEMS.CGST_PERCENT:
                    CalcRow(e.RowIndex);
                    CalcTotals();
                    break;
                case TEX_INVOICEITEMS.IGST_PERCENT:
                    CalcRow(e.RowIndex);
                    CalcTotals();
                    break;
            }
        ...
        void Editgrid_CurrentCellChanged(object sender, EventArgs e)
        {
            ...
            if (editgrid.CurrentCell != null)
            {
                ...
                switch (lookupColName)
                {
                    ...
                    case TEX_INVOICEITEMS.PRODUCT_ID:
                        ShowLookupForm();
                        break;
                    case TEX_INVOICEITEMS.SIZES_ID:
                        ShowLookupForm();
                        break;
                }
            }
        ...
        private void CalcRow(int pRowIndex)
        {
            ...
            decimal vPrice = ConvertTO.Decimal(editgrid[TEX_INVOICEITEMS.PRICE, pRowIndex].Value);

            decimal vQty = ConvertTO.Decimal(editgrid[TEX_INVOICEITEMS.QTY, pRowIndex].Value);

            decimal vtaxableAmount = vQty * vPrice;

            if (txt_taxtype_id.Text == Core.SGST)
            {
                decimal vSgst_per = ConvertTO.Decimal(editgrid[TEX_INVOICEITEMS.SGST_PERCENT, pRowIndex].Value);
                decimal vCgst_per = ConvertTO.Decimal(editgrid[TEX_INVOICEITEMS.CGST_PERCENT, pRowIndex].Value);
                ...
                editgrid[TEX_INVOICEITEMS.SGST_AMOUNT, pRowIndex].Value = (decimal.Round(vSgst_amount, 2)).ToString();
                editgrid[TEX_INVOICEITEMS.CGST_AMOUNT, pRowIndex].Value = (decimal.Round(vCgst_amount, 2)).ToString();
                editgrid[TEX_INVOICEITEMS.IGST_PERCENT, pRowIndex].Value = "";
                editgrid[TEX_INVOICEITEMS.IGST_AMOUNT, pRowIndex].Value = "";
            }
            else
            {
                decimal vIgst_per = ConvertTO.Decimal(editgrid[TEX_INVOICEITEMS.IGST_PERCENT, pRowIndex].Value);
                decimal vIgst_amount = decimal.Round(((vtaxableAmount * vIgst_per) / 100), 2);

                editgrid[TEX_INVOICEITEMS.SGST_AMOUNT, pRowIndex].Value = "";
                editgrid[TEX_INVOICEITEMS.SGST_PERCENT, pRowIndex].Value = "";
                editgrid[TEX_INVOICEITEMS.CGST_AMOUNT, pRowIndex].Value = "";
                editgrid[TEX_INVOICEITEMS.CGST_PERCENT, pRowIndex].Value = "";
                editgrid[TEX_INVOICEITEMS.IGST_AMOUNT, pRowIndex].Value = (decimal.Round(vIgst_amount, 2)).ToString();
            }

            editgrid[TEX_INVOICEITEMS.TAXABLEAMOUNT, pRowIndex].Value = (decimal.Round(vtaxableAmount, 2)).ToString();
        }
        ...
        private void CalcTotals()
        {
            ...
            {
                vTotQty += ConvertTO.Decimal(editgrid[TEX_INVOICEITEMS.QTY, r].Value);

                vTotTaxable += ConvertTO.Decimal(editgrid[TEX_INVOICEITEMS.TAXABLEAMOUNT, r].Value);

                vTotCgst += ConvertTO.Decimal(editgrid[TEX_INVOICEITEMS.CGST_AMOUNT, r].Value);

                vTotSgst += ConvertTO.Decimal(editgrid[TEX_INVOICEITEMS.SGST_AMOUNT, r].Value);

                vTotIgst += ConvertTO.Decimal(editgrid[TEX_INVOICEITEMS.IGST_AMOUNT, r].Value);
            }
        ...
        public void ShowLookupForm()
        {
            ...
            switch (lookupColName)
            {
                ...
                case TEX_INVOICEITEMS.PRODUCT_ID:
                    {
                        ...
                        frmLookup.SelectedPkValue = editgrid[TEX_INVOICEITEMS.PRODUCT_ID, lookupRow].Value;
                        frmLookup.AllowNewEntry = true;
                        frmLookup.AllowEmptySelection = true;
                ...
                case TEX_INVOICEITEMS.SIZES_ID:
                    ...
                    frmLookup.SelectedPkValue = editgrid[TEX_INVOICEITEMS.SIZES_ID, lookupRow].Value;
                    frmLookup.AllowNewEntry = true;
                    frmLookup.AllowEmptySelection = true;
        ...
        void FrmLookup_LookupSelected(object sender, EventArgs e)
        {
            ...
            switch (lookupColName)
            {
                ...
                case TEX_INVOICEITEMS.PRODUCT_ID:
                    if (vEntity == null)
                    {
                        editgrid[TEX_INVOICEITEMS.PRODUCT_ID, lookupRow].Value = "";
                    }
                    else
                    {
                        //if (Customise.Invoice_poid_dcid == true)
                        ...
                        editgrid[TEX_INVOICEITEMS.PRODUCT_ID, lookupRow].Value = Checks.RemoveEscape(vEntity[PRODUCT.PRODUCT_NAME].ToString());
                        editgrid.CurrentCell = editgrid[TEX_INVOICEITEMS.PRODUCT_ID, lookupRow];

                        if (txt_taxtype_id.Text == "SGST")
                        {
                            editgrid[TEX_INVOICEITEMS.SGST_PERCENT, lookupRow].Value = Ccoreplus.GetSgst_Name(vEntity[PRODUCT.PRODUCT_ID] + "");
                            editgrid[TEX_INVOICEITEMS.CGST_PERCENT, lookupRow].Value = Ccoreplus.GetCgst_Name(vEntity[PRODUCT.PRODUCT_ID] + "");
                        }
                        else if (txt_taxtype_id.Text == "IGST")
                        {
                            editgrid[TEX_INVOICEITEMS.IGST_PERCENT, lookupRow].Value = Ccoreplus.GetIgst_Name(vEntity[PRODUCT.PRODUCT_ID] + "");
                        }
                    }
                    break;
                ...
                case TEX_INVOICEITEMS.SIZES_ID:
                    if (vEntity == null)
                    {
                        editgrid[TEX_INVOICEITEMS.SIZES_ID, lookupRow].Value = "";
                    }
                    else
                    {
                        editgrid[TEX_INVOICEITEMS.SIZES_ID, lookupRow].Value = vEntity[SIZES.SIZES_NAME];
                    }
                    break;
            }
        }
        ...
        {
            editgrid[TEX_INVOICEITEMS.PRODUCT_ID, lookupRow].Value = xproduct.GetName();
            editgrid.Focus();
            editgrid.CurrentCell = editgrid[TEX_INVOICEITEMS.SIZES_ID, lookupRow];
        ...
        {
            editgrid[TEX_INVOICEITEMS.SIZES_ID, lookupRow].Value = xsizes.GetName();
            editgrid.Focus();
            editgrid.CurrentCell = editgrid[TEX_INVOICEITEMS.QTY, lookupRow];
