...
namespace CXBILL
{
    public partial class LTex_Invoice 
    {
        ...
        {
            string pkValue = GetSelectedPkValue();
            if (CTex_Invoice_exten.LockStatus(pkValue) == false)
            {
                BuildForm();
                if (pkValue != null)
                {
        ...
        {
            txt_no.LookupColNames = new string[] { TEX_INVOICE.INVOICE_NO };
            txt_no.LookupUpdate += Txt_no_LookupUpdate;
        }
        void Txt_no_LookupUpdate(object sender, EventArgs e)
        {
            txt_no.LookupList = CTex_Invoice_exten.LookupNo(new DAL());
        }
        ...
        {
            txt_party.LookupList = CTex_Invoice_exten.LookupParty(new DAL());
        }
        ...
        private string GetSelectedPkValue()
        {
            ...
            if (vSelRow != null)
            {
                selPkValue = vSelRow.Cells[TEX_INVOICE.TEX_INVOICE_ID].Value.ToString();
            }
            return selPkValue;
        }
        ...
        //public event EventHandler LInvoice_NeedToRefresh;
        //public void RaiseEvent_NeedToRefresh()
        //{
        //    LInvoice_NeedToRefresh?.Invoke(this, new EventArgs());
        //}
