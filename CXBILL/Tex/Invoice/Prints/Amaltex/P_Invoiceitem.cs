...
namespace CXBILL
{
    public class P_InvoiceItem
    {
        ...
        public static Print_invoiceItemsList GetData(string p_id)
        {
            ...
            //
            List<Tex_invoiceitems> inv_item_list = new List<Tex_invoiceitems>();

            if (Customise.Invoice_poid_dcid == true)
            {
                inv_item_list = CTex_Invoiceitems_exten.FKId(p_id, new DAL());
            }

            if ((Customise.Invoice_pono_dcno == true) || (Customise.Invoice_Without_podc == true))
            {
                inv_item_list = CTex_Invoiceitems_exten.FKNo(p_id, new DAL());
            }
            ...
            for (int r = 0; r <= inv_item_list.Count - 1; r++)
            {
                ...
                //
                row.INVOICE_ITEM_ID = inv_item_list[r].Tex_invoiceitems_id;
                row.INVOICE_ID = inv_item_list[r].Tex_invoice_id;
