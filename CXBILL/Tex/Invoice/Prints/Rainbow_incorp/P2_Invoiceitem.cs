...
namespace Billing
{
    public class P2_InvoiceItem
    {
        ...
        public static Print_invoiceItemsList GetData(string p_id)
        {
            ...
            //
            List<Invoice_item> inv_item_list = CInvoice_Item.Invoice_item_Datalist(CInvoice_Item_exten.Foreignkey_invoiceid(p_id));

            for (int r = 0; r <= inv_item_list.Count - 1; r++)
            {
                Print_invoiceitemsRow row = new Print_invoiceitemsRow();
               //
                row.INVOICE_ITEM_ID = inv_item_list[r].Invoice_item_id;
                row.INVOICE_ID = inv_item_list[r].Invoice_id;
                row.SL_NO = (r + 1).ToString();

                if (inv_item_list[r].Po_id == "1")
                {
                    row.PO_ID = "";
                }
                else
                {
                    row.PO_ID = cPo_exten.GetPartypono_poid(inv_item_list[r].Po_id);
                }


                if (inv_item_list[r].Dc_id == "1")
                {
                    row.DC_ID = "";
                }
                else
                {
                    row.DC_ID = CDc_exten.GetDcno_dcid(inv_item_list[r].Dc_id);
                }

                row.PRODUCT_ID = CProduct_exten.GetName_Id(inv_item_list[r].Product_id);

                row.HSN_CODE_ID = CProduct_exten.GetHsncode_Id(inv_item_list[r].Product_id);
                ...
                decimal vcess_per = Global.ToDecimal(inv_item_list[r].Cess_percent);
                ...
                decimal vCESS = decimal.Round(((vTaxableValue * vcess_per) / 100), 2);

                vAmount = vTaxableValue + vCGST + vSGST + vIGST + vCESS;
    ...
    public class Print2_invoiceItemsList : System.ComponentModel.BindingList<Print2_invoiceitemsRow>
    {
    }

    public class Print2_invoiceitemsRow
    {
