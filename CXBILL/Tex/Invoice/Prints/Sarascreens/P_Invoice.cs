...
namespace CodexsunBill
{
    public class P_Invoice
    {
        ...
        {
            Invoice inv_list = CInvoice_exten.PKId(p_id, new DAL());
            ...
            for (int i = 0; i < printcopies.Count; i++)
            {
                ...
                row.SALESTYPE = inv_list.Salestype_id;
                ...
                row.PARTY_NAME = inv_list.Party_id;

                Party _party_list = CParty_exten.SearchName(inv_list.Party_id, new DAL());
                ...
                row.TOTAL_QTY = ConvertTO.DecimalToS(inv_list.Total_qty.ToString());

                row.TAXABLE_VALUE = ConvertTO.Decimal2d(inv_list.Total_taxable_amount.ToString());
                row.TOTAL_CGST = ConvertTO.Decimal2d(inv_list.Total_cgst.ToString());
                row.TOTAL_SGST = ConvertTO.Decimal2d(inv_list.Total_sgst.ToString());
                row.TOTAL_IGST = ConvertTO.Decimal2d(inv_list.Total_igst.ToString());
                ...
                if (inv_list.Ledger_id != null)
                {
                    ...
                    {
                        row.LEDGER = "  "+ CLedger_exten.GetName_Id(inv_list.Ledger_id);
                        row.ADDITIONAL = inv_list.Additional.ToString();
                    }
                    else
                    {
                        row.LEDGER = "";
                ...
                row.ROUNDS = inv_list.Rounds.ToString();
                row.GSTTOTAL = inv_list.Gsttotal.ToString();
                row.GRANDTOTAL = inv_list.Grandtotal.ToString();
