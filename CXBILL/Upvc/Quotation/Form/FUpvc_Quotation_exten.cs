...
namespace CXBILL
{
    public partial class FUpvc_Quotation 
    {
        ...
        private void CalcRow(int pRowIndex)
        {
            ...
            editgrid[UPVC_QUOTATIONITEMS.AREASQ, pRowIndex].Value = (decimal.Round(TotalAreasq, 2)).ToString();


            if (editgrid[UPVC_QUOTATIONITEMS.PRODUCT_ID, pRowIndex].Value != null)
            {
                if (CUpvc_Quotation_exten.Producttype(editgrid[UPVC_QUOTATIONITEMS.PRODUCT_ID, pRowIndex].Value.ToString()) == true)
                {
