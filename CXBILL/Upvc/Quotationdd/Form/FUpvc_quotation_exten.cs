...
namespace CXUPVC
{
    public partial class FUpvc_quotation : UserControl
    {
        ...
        void Editgrid_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            switch (vColName)
            {
                case UPVC_QUOTATIONITEMS.WIDTHS:
                    CalcRow(e.RowIndex);
                    CalcTotals();
                    break;
                case UPVC_QUOTATIONITEMS.HEIGHTS:
                    CalcRow(e.RowIndex);
                    CalcTotals();
                    break;
                case UPVC_QUOTATIONITEMS.QTY:
                    ...
                    CalcTotals();
                    break;
                case UPVC_QUOTATIONITEMS.PRICE:
                    CalcRow(e.RowIndex);
                    CalcTotals();
                    break;
                case UPVC_QUOTATIONITEMS.SGST_PERCENT:
                    CalcRow(e.RowIndex);
                    CalcTotals();
                    break;
                case UPVC_QUOTATIONITEMS.CGST_PERCENT:
                    CalcRow(e.RowIndex);
                    CalcTotals();
                    break;
                case UPVC_QUOTATIONITEMS.IGST_PERCENT:
                    CalcRow(e.RowIndex);
                    CalcTotals();
                    break;
            }
        ...
        void Editgrid_CurrentCellChanged(object sender, EventArgs e)
        {
            ...
            if (editgrid.CurrentCell != null)
            {
                ...
                {
                    case UPVC_QUOTATIONITEMS.PRODUCT_ID:
                        ShowLookupForm();
                        break;
                }
            }
        ...
        private void CalcRow(int pRowIndex)
        {
            ...
            decimal vWidths = ConvertTO.Decimal(editgrid[UPVC_QUOTATIONITEMS.WIDTHS, pRowIndex].Value);
            decimal vHeights = ConvertTO.Decimal(editgrid[UPVC_QUOTATIONITEMS.HEIGHTS, pRowIndex].Value);

            decimal vAreasq = decimal.Round(((vWidths / vCalculation) * (vHeights / vCalculation)),2);

            editgrid[UPVC_QUOTATIONITEMS.AREASQ, pRowIndex].Value = (decimal.Round(vAreasq, 2)).ToString();

            decimal vPrice = ConvertTO.Decimal(editgrid[UPVC_QUOTATIONITEMS.PRICE, pRowIndex].Value);

            decimal vrateperpcs = vAreasq * vPrice;

            decimal vQty = ConvertTO.Decimal(editgrid[UPVC_QUOTATIONITEMS.QTY, pRowIndex].Value);


            if (editgrid[UPVC_QUOTATIONITEMS.PRODUCT_ID, pRowIndex].Value != null)
            {
                if (CUpvc_invoice_exten.Producttype(editgrid[UPVC_QUOTATIONITEMS.PRODUCT_ID, pRowIndex].Value.ToString()) == true)
                {
            ...
            {
                decimal vSgst_per = ConvertTO.Decimal(editgrid[UPVC_QUOTATIONITEMS.SGST_PERCENT, pRowIndex].Value);
                decimal vCgst_per = ConvertTO.Decimal(editgrid[UPVC_QUOTATIONITEMS.CGST_PERCENT, pRowIndex].Value);
                ...
                editgrid[UPVC_QUOTATIONITEMS.SGST_AMOUNT, pRowIndex].Value = (decimal.Round(vSgst_amount, 2)).ToString();
                editgrid[UPVC_QUOTATIONITEMS.CGST_AMOUNT, pRowIndex].Value = (decimal.Round(vCgst_amount, 2)).ToString();
                editgrid[UPVC_QUOTATIONITEMS.IGST_PERCENT, pRowIndex].Value = "";
                editgrid[UPVC_QUOTATIONITEMS.IGST_AMOUNT, pRowIndex].Value = "";
            }
            else
            {
                decimal vIgst_per = ConvertTO.Decimal(editgrid[UPVC_QUOTATIONITEMS.IGST_PERCENT, pRowIndex].Value);
                decimal vIgst_amount = decimal.Round(((vtaxableAmount * vIgst_per) / 100), 2);

                editgrid[UPVC_QUOTATIONITEMS.SGST_AMOUNT, pRowIndex].Value = "";
                editgrid[UPVC_QUOTATIONITEMS.SGST_PERCENT, pRowIndex].Value = "";
                editgrid[UPVC_QUOTATIONITEMS.CGST_AMOUNT, pRowIndex].Value = "";
                editgrid[UPVC_QUOTATIONITEMS.CGST_PERCENT, pRowIndex].Value = "";
                editgrid[UPVC_QUOTATIONITEMS.IGST_AMOUNT, pRowIndex].Value = (decimal.Round(vIgst_amount, 2)).ToString();
            }

            editgrid[UPVC_QUOTATIONITEMS.TAXABLEAMOUNT, pRowIndex].Value = (decimal.Round(vtaxableAmount, 2)).ToString();
        }
        ...
        private void CalcTotals()
        {
            ...
            {
                vTotAreasq += ConvertTO.Decimal(editgrid[UPVC_QUOTATIONITEMS.AREASQ, r].Value);

                vTotQty += ConvertTO.Decimal(editgrid[UPVC_QUOTATIONITEMS.QTY, r].Value);

                vTotTaxable += ConvertTO.Decimal(editgrid[UPVC_QUOTATIONITEMS.TAXABLEAMOUNT, r].Value);

                vTotCgst += ConvertTO.Decimal(editgrid[UPVC_QUOTATIONITEMS.CGST_AMOUNT, r].Value);

                vTotSgst += ConvertTO.Decimal(editgrid[UPVC_QUOTATIONITEMS.SGST_AMOUNT, r].Value);

                vTotIgst += ConvertTO.Decimal(editgrid[UPVC_QUOTATIONITEMS.IGST_AMOUNT, r].Value);
            }
                ...
                case UPVC_QUOTATIONITEMS.PRODUCT_ID:
                    {
                        ...
                        frmLookup.SelectedPkValue = editgrid[UPVC_QUOTATIONITEMS.PRODUCT_ID, lookupRow].Value;
        ...
        void FrmLookup_LookupSelected(object sender, EventArgs e)
        {
            ...
            switch (lookupColName)
            {
                case UPVC_QUOTATIONITEMS.PRODUCT_ID:
                    if (vEntity == null)
                    {
                        editgrid[UPVC_QUOTATIONITEMS.PRODUCT_ID, lookupRow].Value = "";
                    }
                    else
                    {
                        editgrid[UPVC_QUOTATIONITEMS.PRODUCT_ID, lookupRow].Value = vEntity[PRODUCT.PRODUCT_NAME];
                        editgrid[UPVC_QUOTATIONITEMS.SGST_PERCENT, lookupRow].Value = Ccoreplus.GetSgst_Name(vEntity[PRODUCT.PRODUCT_NAME] + "");
                        editgrid[UPVC_QUOTATIONITEMS.CGST_PERCENT, lookupRow].Value = Ccoreplus.GetCgst_Name(vEntity[PRODUCT.PRODUCT_NAME] + "");
                    }
                    break;
            }
        }
        ...
        {
            editgrid[UPVC_QUOTATIONITEMS.PRODUCT_ID, lookupRow].Value = xproduct.GetName();
            editgrid.Focus();
            editgrid.CurrentCell = editgrid[UPVC_QUOTATIONITEMS.QTY, lookupRow];
