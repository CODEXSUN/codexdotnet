...
using D_LabourItem = CXBILL_EXTEN.Labour.Prints.D_LabourItem;
using LABOURITEMS = CXBILL_EXTEN.Labour.Const.LABOURITEMS;
using P_Labour = CXBILL_EXTEN.Labour.Prints.P_Labour;
    ...
    public partial class FLabour 
    {
        ...
        private void SetTaxtype()
        {
            if (txt_taxtype_id.Text == Core.SGST)
            {
                ...
                editgrid.Columns[LABOURITEMS.SGST_PERCENT].Visible = true;
                editgrid.Columns[LABOURITEMS.CGST_PERCENT].Visible = true;
                editgrid.Columns[LABOURITEMS.IGST_PERCENT].Visible = false;
                RefreshRowtotal();
                this.Refresh();
            }
            else if (txt_taxtype_id.Text == Core.IGST)
            {
                ...
                editgrid.Columns[LABOURITEMS.SGST_PERCENT].Visible = false;
                editgrid.Columns[LABOURITEMS.CGST_PERCENT].Visible = false;
                editgrid.Columns[LABOURITEMS.IGST_PERCENT].Visible = true;
                RefreshRowtotal();
                this.Refresh();
            }
        }
        ...
        public event EventHandler FLabour_NeedToRefresh;

        public void Form_NeedToRefresh()
        {
            if (FLabour_NeedToRefresh != null)
            {
                FLabour_NeedToRefresh(this, new EventArgs());
            }
        }
        ...
        private void Print_copies()
        {
            ...
            P_Labour.Print(frpt.reportViewer1, (vId), printdata);
            //P_LabourItem.Print(frpt.reportViewer1, (vId));
            frpt.Show();
            print_panel.Hide();
            this.Hide();
