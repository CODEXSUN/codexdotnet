...
namespace CXBILL_EXTEN.Labour.Prints
{
    public class P_Labour
    {
        ...
        {
            vPId = pP_id;
            Print_LabourList data = PrintLabourExchange.GetData(pP_id, printdata);

            LocalReport localReport = pRptViewer.LocalReport;

            D_Labour.Build();

            localReport.ReportPath = Application.StartupPath + @"\PRINTS\P_Labour.rdlc"; //(1)//
        ...
        static void localReport_SubreportProcessing(object sender, SubreportProcessingEventArgs e)
        {
            switch (e.DataSourceNames[0])
            {
                case "MyDataSource_Items":
                    try
                    {
                        Print_invoiceItemsList data_AT = P_LabourItem.GetData(vPId);
        ...
        //private static Print_LabourList GetData(string p_id, List<PrintCopies> printcopies)
        //{
        //    Labour invoice = CLabour_exten.PKId(p_id);

        //    Print_LabourList list = new Print_LabourList();

        //    for (int i = 0; i < printcopies.Count; i++)
        //    {
        //        Print_LabourRow row = new Print_LabourRow();
        ...
        //        row.LABOUR_ID = p_id;
        //        row.LABOUR_NO = ".: " + invoice.Labour_no.ToString();
        ...
        //        List<Labouritems> klist = CLabouritems_exten.FKId(p_id);
    ...
    public class Print_LabourList : System.ComponentModel.BindingList<Print_Labour>
    {
    }
