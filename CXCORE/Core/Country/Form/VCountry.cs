    ...
    public partial class FCountry : UserControl
    {
        ...
        public void SetAction(BtnEvent pAction, string pPkValue)
        {
            ...
            switch (fAction)
            {
                case BtnEvent.Open:
                    btn_save.Text = "&CLOSE";
                    obj = CCountry_exten.PKId(pPkValue, new DAL());
        ...
        {
            vId = obj.Country_id;
            txt_country_name.Text = obj.Country_name;
        ...
        private void EnableControls(bool pEnabled)
        {
            ...
            txt_country_name.Enabled = !pReadOnly;
                ...
                switch (fAction)
                {
                    ...
                    case BtnEvent.New:
                        try
                        {
                            ...
                            CCountry.Insert(vData, dalsession);
                            dalsession.Commit();
                    ...
                    case BtnEvent.Edit:
                        try
                        {
                            ...
                            dalsession = new DAL();
                            dalsession.BeginTransaction();
                            CCountry.Update(vData, dalsession);
                            dalsession.Commit();
                    ...
                    case BtnEvent.Delete:
                        try
                        {
                            dalsession = new DAL();
                            dalsession.BeginTransaction();
                            CCountry.Delete(vId, dalsession);
                            dalsession.Commit();
        ...
        private Country CollectData()
        {
            Country fobj = new Country()
            {
                Country_id = vId,
                Country_name = txt_country_name.Text,
        ...
        {
            if (txt_country_name.Text.Trim().Length == 0)
            {
                MessageBox.Show(this.FindForm(), "Country name should not Empty ! ", "Warning...", MessageBoxButtons.OK, MessageBoxIcon.Warning);
