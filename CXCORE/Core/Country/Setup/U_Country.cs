    ...
    public static class U_Country
    {
        #region[]
            ...
            public static void GetUpdated(string OldDB)
            {
                ...
                string tablename = COUNTRY.COUNTRY_TBL;
        ...
        #region[Get Old data]
            ...
            private static Country GetOldData(int id, string OldDB)
            {
                string q = "USE " + OldDB + "; ";
                q += "SELECT * FROM " + COUNTRY.COUNTRY_TBL + " WHERE " + COUNTRY.COUNTRY_ID + " = '" + id + "'; ";
...
#region[Check data]
            ...
            private static bool CheckData(Country obj)
            {
                string q = "USE CODEXSUN_DB; ";
                q += "SELECT * FROM " + COUNTRY.COUNTRY_TBL + " WHERE " + COUNTRY.COUNTRY_NAME + " = '" + obj.Country_name + "'; ";

                using (IDataReader redr = new DAL().Listreader(q))
                {
                    while (redr.Read() == true)
                    {
                        obj.Country_id = redr[COUNTRY.COUNTRY_ID].ToString();

                        CCountry.Update(obj, new DAL());
        ...
        #region[Insert data]
            ...
            private static void InsertData(Country obj)
            {
                if (CheckData(obj) == false)
                {
                    CCountry.Insert(obj, new DAL());
                };
