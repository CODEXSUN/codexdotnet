...
namespace CXCORE
{
    public static class U_Ledgergroup
    {
        #region[]
            ...
            public static void GetUpdated(string OldDB)
            {
                ...
                string tablename = LEDGERGROUP.LEDGERGROUP_TBL;
        ...
        #region[Get Old data]
            ...
            private static Ledgergroup GetOldData(int id, string OldDB)
            {
                string q = "USE " + OldDB + "; ";
                q += "SELECT * FROM " + LEDGERGROUP.LEDGERGROUP_TBL + " WHERE " + LEDGERGROUP.LEDGERGROUP_ID + " = '" + id + "'; ";
...
#region[Check data]
            ...
            private static bool CheckData(Ledgergroup obj)
            {
                string q = "USE CODEXSUN_DB; ";
                q += "SELECT * FROM " + LEDGERGROUP.LEDGERGROUP_TBL + " WHERE " + LEDGERGROUP.LEDGERGROUP_NAME + " = '" + obj.Ledgergroup_name + "'; ";

                using (IDataReader redr = new DAL().Listreader(q))
                {
                    while (redr.Read() == true)
                    {
                        obj.Ledgergroup_id = redr[LEDGERGROUP.LEDGERGROUP_ID].ToString();

                        CLedgergroup.Update(obj, new DAL());
        ...
        #region[Insert data]
            ...
            private static void InsertData(Ledgergroup obj)
            {
                if (CheckData(obj) == false)
                {
                    CLedgergroup.Insert(obj, new DAL());
                };
