// Version : 2.2.0.0 dt : 15-03-2019
// Auto Generated
// last update : 29-03-2019 09:06:45 AM

using System.Data;
using CXLIB;
using System.Collections.Generic;

namespace CXCORE
{
    public class CPincode_exten
    {
        #region[Entity Data]

        private static Pincode EntityData(string q)
        {
            using (IDataReader redr = new DAL().Listreader(q))
            {
                while (redr.Read() == true)
                {
                    Pincode obj = new Pincode()
                    {
                        Pincode_id = redr[PINCODE.PINCODE_ID].ToString(),
                        Pincode_name = redr[PINCODE.PINCODE_NAME].ToString(),
                        Notes = redr[PINCODE.NOTES].ToString(),
                        Active_id = redr[PINCODE.ACTIVE_ID].ToString(),
                        User_id = redr[USER.USER_NAME].ToString()
                    };
                    return obj;
                }
                redr.Close();
                return new Pincode();
            }
        }

        #endregion[Entity Data]

        #region[Entity List]

        private static List<Pincode> EntityList(string q)
        {
            List<Pincode> list = new List<Pincode>();

            using (IDataReader redr = new DAL().Listreader(q))
            {
                while (redr.Read() == true)
                {
                    Pincode obj = new Pincode()
                    {
                        Pincode_id = redr[PINCODE.PINCODE_ID].ToString(),
                        Pincode_name = redr[PINCODE.PINCODE_NAME].ToString(),
                        Notes = redr[PINCODE.NOTES].ToString(),
                        Active_id = redr[PINCODE.ACTIVE_ID].ToString(),
                        User_id = redr[USER.USER_NAME].ToString()
                    };

                    list.Add(obj);
                }

                redr.Close();
            }
            return list;
        }

        #endregion[Entity List]

        #region[Active]

        public static List<Pincode> Active(DAL DAL)
        {
            string q = " SELECT " + PINCODE.PINCODE_TBL + ".* \r\n";
            q += ", " + USER.USER_TBL + "." + USER.USER_NAME + " \r\n";
            q += " FROM  " + PINCODE.PINCODE_TBL + " \r\n";
            q += " INNER JOIN  " + USER.USER_TBL + "\r\n";
            q += " ON(" + PINCODE.PINCODE_TBL + "." + USER.USER_ID + "  = " + USER.USER_TBL + "." + USER.USER_ID + ")\r\n";
            q += " WHERE " + PINCODE.PINCODE_TBL + "." + PINCODE.ACTIVE_ID + "  = '" + Core.Active + "' \r\n";
            //q += " AND NOT " + PINCODE.PINCODE_TBL + "." + PINCODE.PINCODE_ID + " = '" + Core.One + "' \r\n";
            q += " ORDER BY  " + PINCODE.PINCODE_TBL + "." + PINCODE.PINCODE_NAME + ";\r\n";

            return EntityList(q);
        }

        #endregion[Active]

        #region[NotActive]

        public static List<Pincode> NotActive(DAL DAL)
        {
            string q = " SELECT " + PINCODE.PINCODE_TBL + ".* \r\n";
            q += ", " + USER.USER_TBL + "." + USER.USER_NAME + " \r\n";
            q += " FROM  " + PINCODE.PINCODE_TBL + " \r\n";
            q += " INNER JOIN  " + USER.USER_TBL + "\r\n";
            q += " ON(" + PINCODE.PINCODE_TBL + "." + USER.USER_ID + "  = " + USER.USER_TBL + "." + USER.USER_ID + ")\r\n";
            q += " WHERE " + PINCODE.PINCODE_TBL + "." + PINCODE.ACTIVE_ID + "  = '" + Core.NotActive + "' \r\n";
            //q += " AND NOT " + PINCODE.PINCODE_TBL + "." + PINCODE.PINCODE_ID + " = '" + Core.One + "' \r\n";
            q += " ORDER BY  " + PINCODE.PINCODE_TBL + "." + PINCODE.PINCODE_NAME + ";\r\n";

            return EntityList(q);
        }

        #endregion[NotActive]

        #region[Primary Key]

        public static Pincode PKId(string v, DAL DAL)
        {
            string q = " SELECT " + PINCODE.PINCODE_TBL + ".* \r\n";
            q += ", " + USER.USER_TBL + "." + USER.USER_NAME + " \r\n";
            q += " FROM  " + PINCODE.PINCODE_TBL + " \r\n";
            q += " INNER JOIN  " + USER.USER_TBL + "\r\n";
            q += " ON(" + PINCODE.PINCODE_TBL + "." + USER.USER_ID + "  = " + USER.USER_TBL + "." + USER.USER_ID + ")\r\n";
            q += " WHERE " + PINCODE.PINCODE_TBL + "." + PINCODE.PINCODE_ID + "  = '" + v + "' \r\n";

            return EntityData(q);
        }

        #endregion[Primary Key]

        #region[Search by name]

        public static Pincode SearchName(string v, DAL DAL)
        {
            string q = " SELECT " + PINCODE.PINCODE_TBL + ".* \r\n";
            q += ", " + USER.USER_TBL + "." + USER.USER_NAME + " \r\n";
            q += " FROM  " + PINCODE.PINCODE_TBL + " \r\n";
            q += " INNER JOIN  " + USER.USER_TBL + "\r\n";
            q += " ON(" + PINCODE.PINCODE_TBL + "." + USER.USER_ID + "  = " + USER.USER_TBL + "." + USER.USER_ID + ")\r\n";
            q += " WHERE " + PINCODE.PINCODE_TBL + "." + PINCODE.PINCODE_NAME + "  = '" + v + "' \r\n";
            q += " ORDER BY  " + PINCODE.PINCODE_TBL + "." + PINCODE.PINCODE_NAME + ";\r\n";

            return EntityData(q);
        }

        #endregion[Search by name]

        #region[Look up]

        public static DataTable GetforLookup(DAL DAL)
        {
            string q = " SELECT " + PINCODE.PINCODE_TBL + ".* \r\n";
            q += ", " + USER.USER_TBL + "." + USER.USER_NAME + " \r\n";
            q += " FROM  " + PINCODE.PINCODE_TBL + " \r\n";
            q += " INNER JOIN  " + USER.USER_TBL + "\r\n";
            q += " ON(" + PINCODE.PINCODE_TBL + "." + USER.USER_ID + "  = " + USER.USER_TBL + "." + USER.USER_ID + ")\r\n";
            q += " WHERE " + PINCODE.PINCODE_TBL + "." + PINCODE.ACTIVE_ID + "  = '" + Core.Active + "' \r\n";
            //q += " AND NOT " + PINCODE.PINCODE_TBL + "." + PINCODE.PINCODE_ID + " = '" + Core.One + "' \r\n";
            q += " ORDER BY  " + PINCODE.PINCODE_TBL + "." + PINCODE.PINCODE_NAME + ";\r\n";

            return DAL.Listdata(q);
        }

        #endregion[Look up]

        #region[Get id - Name]

        public static string GetName_Id(string _pId)
        {
            Pincode obj = PKId(_pId, new DAL());

            if (obj != null)
            {
                return obj.Pincode_name;
            }
            return "";
        }

        public static string GetId_Name(string _pName)
        {
            Pincode obj = SearchName(_pName, new DAL());

            if (obj != null)
            {
                return obj.Pincode_id;
            }
            return "";
        }

        #endregion[Get id - Name]

    }//cls
}//ns
