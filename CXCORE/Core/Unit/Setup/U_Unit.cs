...
namespace CXCORE
{
    public static class U_Unit
    {
        ...
        public static void GetUpdated(string OldDB)
        {
            ...
            string tablename = UNIT.UNIT_TBL;
        ...
        private static Unit GetOldData(int id, string OldDB)
        {
            string q = "USE " + OldDB + "; ";
            q += "SELECT * FROM " + UNIT.UNIT_TBL + " WHERE " + UNIT.UNIT_ID + " = '" + id + "'; ";
        ...
        private static bool CheckData(Unit obj)
        {
            string q = "USE CODEXSUN_DB; ";
            q += "SELECT * FROM " + UNIT.UNIT_TBL + " WHERE " + UNIT.UNIT_NAME + " = '" + obj.Unit_name + "'; ";

            using (IDataReader redr = new DAL().Listreader(q))
            {
                while (redr.Read() == true)
                {
                    obj.Unit_id = redr[UNIT.UNIT_ID].ToString();

                    CUnit.Update(obj, new DAL());
        ...
        private static void InsertData(Unit obj)
        {
            if (CheckData(obj) == false)
            {
                CUnit.Insert(obj, new DAL());
            };
