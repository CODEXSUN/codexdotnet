...
namespace CXERP
{
    public partial class LInward 
    {
        ...
        {
            string pkValue = GetSelectedPkValue();
            if (CInward_exten.LockStatus(pkValue) == false)
            {
                BuildForm();
                if (pkValue != null)
                {
        ...
        {
            txt_no.LookupColNames = new string[] { INWARD.INWARD_NO };
            txt_no.LookupUpdate += Txt_no_LookupUpdate;
        }
        void Txt_no_LookupUpdate(object sender, EventArgs e)
        {
            txt_no.LookupList = CInward_exten.LookupNo();
        }
        ...
        {
            txt_party.LookupList = CInward_exten.LookupParty();
        }
        ...
        private string GetSelectedPkValue()
        {
            ...
            if (vSelRow != null)
            {
                selPkValue = vSelRow.Cells[INWARD.INWARD_ID].Value.ToString();
            }
            return selPkValue;
        }
        ...
        void VInward_NeedToRefresh(object sender, EventArgs e)
        {
            List_Option(ListOption.active);
        }
        //public event EventHandler LInward_NeedToRefresh;
        //public void RaiseEvent_NeedToRefresh()
        //{
        //    LInward_NeedToRefresh?.Invoke(this, new EventArgs());
        //}
